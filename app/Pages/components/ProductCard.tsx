...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className="border rounded-lg overflow-hidden sm:w-[250px] w-[180px] sm:h-[370px] h-[350px] bg-white flex flex-col">
      <img src={image.src} alt={title} className="w-full sm:h-64 h-56 object-cover" />
      <div className="p-4 flex flex-col justify-between flex-grow">
        <div>
          <div className="flex justify-between items-center">
            <h2 className="font-bold sm:text-lg text-sm mb-2 capitalize ">{title}</h2>
            {isEco && (
        ...
        <div className="flex justify-between ">
          <span className="flex items-center gap-2 sm:text-md text-xs">
            ...
            <svg
              ...
              fill="gold"
              viewBox="0 0 256 256"
              className="w-4 h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-6 lg:h-6 xl:w-6 xl:h-6 2xl:w-4 2xl:h-4"
            >
              <path
          ...
          <p className="text-gray-700 sm:text-md text-xs">${price.toFixed(2)}</p>
        </div>
      </div>
    </div>
  );
